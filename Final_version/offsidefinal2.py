...
cap = cv2.VideoCapture(r'offsidefinal2_cut.mp4')
...
while(cap.isOpened()):
    ...
    if ret:
        ...
        lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 180, lines=100, minLineLength=600, maxLineGap=80)
        ...
        if lines is not None:
            ...
            y1_min = 10000
            y2_min = 10000
            x1_min_sideline = 10000
            x2_min_sideline = 10000
            y1_min_sideline = 10000
            y2_min_sideline = 10000
            for line in lines:
                x1, y1, x2, y2 = line[0]
                if ((x2 - x1) + 300 < (y2 - y1)):
    ...
    # retrieves only the extreme outer contours
        ...
        if counter % 20 == 0 and counter < 210:
            # print("bingo")
            # library of color
            whiteLower = (0, 0, 205)
...
#  -------------------------------algorithm--------------------------------------
        ...
        print("attacker location:", location_red_2d)
        print(box)
        ...
        if box is not None:
            ...
            for location in location_red_2d:
                leng = abs(location[0] - box[0])**2 + abs(location[1] - box[1])**2
                leng = np.sqrt(leng)
                print(leng)
                if leng < min_leng:
                    min_leng = leng
                    index = coun
                coun = coun + 1
            print("final result of length", index, min_leng)
            if min_leng < 100:
                print("ball attached")
                flag_ball_detach = False
                record_temp = index
                ...
                if flag_offside_position:
                    prejudge = True
                pass
            print("record:",record, "detach:", flag_ball_detach, "index(ball catcher): ", index
                  , "prejudge",prejudge)
            if record is not None and not flag_ball_detach:  # when somebody get the ball
                if record != index and index == 0 and prejudge:
                    # print("offside")
                    flag_offside = True
                pass
        info = [
