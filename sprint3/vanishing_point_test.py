...
cap = cv2.VideoCapture("offsidewhite1.mp4")
...
while(1):
    ...
    lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 100, lines=100, minLineLength=700, maxLineGap=150)
    # line_img,lines = cv2.HoughLinesP(edges2, 1, np.pi / 180, 80, lines=80, minLineLength=220, maxLineGap=18)

    ##new video
    # lines = cv2.HoughLinesP(edges2, 1, np.pi / 180, 30, lines=80, minLineLength=120, maxLineGap=25)
    # line_img,lines = cv2.HoughLinesP(edges2, 1, np.pi / 180, 30, lines=80, minLineLength=120, maxLineGap=25)
...
##------------------------End For Detect Line-------------------##
    ...
    # line_img, lines = hough_lines(edges2, 1, np.pi / 180, 30, 120, 25)
    #
    # # while(1):
    # VP();
